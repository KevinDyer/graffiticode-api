(function () {
  ...
  const errorNotWellFormed = "CSV is not well formed";
  const errorEmpty = "Please upload a file or type in something.";
  const errorEmptyHeader = "Could not detect header. Ensure first row cotains your column headers.";
  const separators = [",", ";", "\t"];
  const pegjsSeparatorNames = {
    ",": "comma",
    ";": "semicolon",
    "\t": "tab"
  };
  ...
  function convert (csv, options) {
    ...
    try {
      ...
      const start = csv.lastIndexOf("\n", error.offset);
      const end = csv.indexOf("\n", error.offset);
      const line = csv.substring(start >= -1 ? start : 0, end > -1 ? end : csv.length);
    ...
    console.log("Generating JSON for " + a.length + " records");
    const	json = options.hash ? {} : [];
    for (let l = 0; l < a.length; l++) {
      const row = {};
      var hashKey;
      for (let i = 0; i < keys.length; i++) {
        const value = (a[l][i] || "").trim().replace(/(^")|("$)/g, "");
